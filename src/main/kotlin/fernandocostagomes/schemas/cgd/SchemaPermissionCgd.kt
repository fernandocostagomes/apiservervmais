...
data class Permission(
    ...
    var data: String )
    class ServicePermissionCgd(private val connection: Connection): SchemaInterface {

        companion object {
            private const val TABLE = "v_cgd_permission"
            private const val COLUMN_ID = "v_cgd_permission_id"
            private const val COLUMN_NAME = "v_cgd_permission_name"
            private const val COLUMN_DESCRIPTION = "v_cgd_permission_description"
            private const val COLUMN_DATE = "v_cgd_permission_date"

            private const val COLUMN_ID_QUERY = "$COLUMN_ID SERIAL PRIMARY KEY, "
            private const val COLUMN_NAME_QUERY = "$COLUMN_NAME VARCHAR(50), "
            private const val COLUMN_DESCRIPTION_QUERY = "$COLUMN_DESCRIPTION VARCHAR(50), "
        ...
        private fun getStatement(pStatement: PreparedStatement, pObj: Any): PreparedStatement {
            ...
            pStatement.setString(2, pObj.description)
        ...
        override suspend fun list(): List<Permission> = withContext( Dispatchers.IO ) {
            ...
            val permissionMutableList = mutableListOf<Permission>()

            while (resultSet.next()) {
                permissionMutableList.add( getResultset( resultSet ) )
            }

            if (permissionMutableList.isNotEmpty())
                return@withContext permissionMutableList
            else
                throw Exception(SchemaUtils.RECORD_NOT_FOUND)
        }
